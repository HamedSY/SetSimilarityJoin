...
object Main extends App {
  ...
  val algorithmHandler = new PPJoinHandler(threshold = 0.7)
  // Address of the dataset(s)
  // Use "executeRSJoin" if you have two datasets

  val norStartTime = System.nanoTime()
  val norResults = executer.executeSelfJoin("data/bms-pos-5p-sample.txt", algorithmHandler)
  ...
  val farResults = executer.executeSelfJoinWithFAR("data/bms-pos-5p-sample.txt", algorithmHandler, 30)
